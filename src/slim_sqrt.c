...
void slim_sqrt(double *Y, double *X, double *XX, double *beta, int *n, int *d, double *rho, int *ite_cnt_ext, int *ite_cnt_int1, int *ite_cnt_int2, double *lambda, int * nnlambda, int *max_ite, double *prec, int * intercept)
{
    ...
    double gamma,alp_tild_sq,alp_th, beta_sum1,beta_dif_sum1,beta_sum2,gap_int,ilambda,ilambda_tmp,tmp1,tmp2,beta_dif,eps,eps1,eps2,max_dif,beta_out1,mu_dif;
    ...
    for(m=0; m<nlambda; m++) {
        ...
        while(max_dif > eps1 && ite_ext < max_ite1){
            ...
            alp_th = 1-gamma/sqrt(alp_tild_sq);
