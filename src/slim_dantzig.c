...
void slim_dantzig(double *Y, double *X, double *XX, double *beta, int *n, int *d, double *rho, int *ite_cnt_ext, int *ite_cnt_int1, int *ite_cnt_int2, double *lambda, int * nnlambda, int *max_ite, double *prec, int * intercept)
{
    ...
    double gamma,alp_dif, beta_sum1,beta_dif_sum1,beta_sum2,gap_int,ilambda,tmp1,tmp2,beta_dif,eps,eps1,eps2,max_dif,beta_out1,mu_dif;
    ...
    double *X_beta = (double*) malloc(dim*sizeof(double));
    ...
    for(j=0; j<dim; j++) {
        ...
        gamma_col[j] = ndata/((*rho)*XX_diag[j]);
    }
    for(j=0; j<dim; j++) {
        alp[j] = 0;
        mu[j] = 0;
