#----------------------------------------------------------------------------------#
# Package: flare                                                                   #
# flare.slim.lad(): Regression with Lad Lasso()                                    #
# Author: Xingguo Li                                                               #
...
flare.slim.lad <- function(Y, X, lambda, nlambda, n, d, maxdf, rho, max.ite, prec)
{
  cat("LAD Lasso regression.\n")
  XX = t(X)%*%X
  ...
  intercept=0
  str=.C("slim_lad", as.double(Y), as.double(X), as.double(XX), 
